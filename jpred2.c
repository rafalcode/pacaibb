/* FOlloiwn gfrom jpred0.c ... I foudn it was getting complicated.
 * so I'm cutting it down here. No point of interest for the time being */
#include <math.h>
#include<stdlib.h>
#include<stdio.h>
...
void lay0(cairo_t *cr, rpla_t r, larr_t *la)
{
    ...
    //indent!
    int ii;
    ii = pango_layout_get_indent(playout);
    printf("ind before set = %i\n", ii); 
    pango_layout_set_indent(playout, 30*PANGO_SCALE);
    ii = pango_layout_get_indent(playout);
    printf("ind after set = %i\n", ii); 
...
int main (int argc, char *argv[])
{
   ...
   // cairo_fill(cr);
    ...
    cairo_destroy (cr);
    cairo_surface_write_to_png(surf2, "jpred2.png");
