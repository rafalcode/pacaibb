/* taken from speeb0.c
 * instcap0.c
 * Insert Text Caption 0
 * this just puts a text rectangle that is all */
#include<stdio.h>
#include<math.h>
...
typedef struct /* rpla_t: rectanglemetn placement type xy and top left pos, w and h width and height. */
{
    float x,y;
    float w,h;
    float lm, rm; // left marg and right margin .. for the text!
} rpla_t;
...
void prtcoords(rpla_t *r, float *mx, float *my)
{
    printf("Starting point of spike=(%4.4f,%4.4f)\n", *mx, *my);
    printf("xcoord of text margin=(%4.4f,%4.4f)\n", r->lm, r->rm);
    printf("topleft point of rect=(%4.4f,%4.4f)\n", r->x, r->y);
    printf("Extent of rect=(%4.4f,%4.4f)\n", r->w, r->h);
}

void popcoords(rpla_t *r0, float *mx, float *my, larr_t *la) // populate the coords
{
    char *colon=NULL;
    colon=strchr(la->l[3], '=');
    r0->x=atoi(colon+1);
...
void lay0(cairo_t *cr, rpla_t *r, char *thetext)
{
    ...
    if(hspa <.0) {
        ...
        pango_layout_set_width(playout, ourscale); // very bizarre integer hereis like in the 100K's! How can that be a width?
    ...
    float linespacing=0.9;
    pango_layout_set_line_spacing(playout, linespacing);
...
int main(int argc, char* argv[])
{
    if(argc!=2) {
        ...
        printf("2) xcoord of originating mouth (spike)\n");
        printf("3) ycoord of originating mouth (spike)\n");
    ...
    rpla_t *r0 = malloc(sizeof(rpla_t));
    float mx, my;
    popcoords(r0, &mx, &my, la);
   ...
   cairo_move_to(cr, r0->x, r0->y); //across left
   cairo_line_to(cr, r0->x + r0->w, r0->y);
   cairo_line_to(cr, r0->x + r0->w, r0->y + r0->h);
   ...
   printf("Output png saved to %s\n", outn); 
   // status = cairo_surface_write_to_png(surf0, outn);
   // cairo_surface_write_to_png(surf0, outn);
   cairo_surface_write_to_png(surf0, outn);
