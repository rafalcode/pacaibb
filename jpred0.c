...
#define LST 3 // line stroke width

int main (int argc, char *argv[])
{
    
    if(argc!=4) {
        printf("Please give 3 arguments: 1) input image file (must be JPG) 2) xcoord point of interest 3) ycoord POI\n");
        exit(EXIT_FAILURE);
    }
    ...
    // now we split the space into an reduced image part on left and an annotation on right. I.e. it's a width operation.
    float iredfrac=IRF; 
    ...
    cairo_surface_t *surf2 = cairo_pdf_surface_create ("jpred0.pdf", lwh[LWI], lwh[LHI]);
    ...
    cairo_surface_write_to_png(surf2, "jpred0.png");
