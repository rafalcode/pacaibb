...
void lay0(cairo_t *cr, rpla_t r, larr_t *la)
{
    ...
    int ystartpos=atoi(la->l[2]);

    /* select font */
	PangoFontDescription *desc= pango_font_description_from_string(la->l[5]);
    if(desc == NULL) {
    ...
    char *parstr=calloc(1+la->lz[6], sizeof(char)); // the actual text to show.

    PangoAttrList *attlst;
    pango_parse_markup(la->l[6], -1, '\0', &attlst, &parstr, NULL, NULL);
...
int main(int argc, char* argv[])
{
    ...
    if (status != CAIRO_STATUS_SUCCESS) {
        ...
        printf("Could not save to png, \"htblkz2.png\"\n");
        return 1;
    }
